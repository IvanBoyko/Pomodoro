...
- When integrating a new framework, add observable logging on auth, schedule/configure, and key state transitions BEFORE the first device test. Silent `try?` / `catch {}` swallow exactly the errors that take a device cycle to surface; one device round-trip costs more than a few `print` lines. Same rule for any device-only repro where the cause is a hypothesis, not verified: instrument first, ship second. If a hypothesis-justified fix fails the first device test, the next step is logs — not a second hypothesis.
- When a framework exposes multiple parallel state surfaces (e.g. AlarmKit's authoritative `Alarm.state` vs the rendering-side `Activity.content.state.mode`), identify the authoritative surface for each property before reading. Parallel surfaces are updated asynchronously, so divergence windows are real — reading paused/running from the rendering surface during a transition will give the wrong answer. Reading from the authoritative source is preferable to a safety net at the point of consequence. SPEC.md records the AlarmKit case explicitly.
...
- If an async stream (`AsyncSequence`, Combine `Publisher`, NotificationCenter, etc.) already supplies a piece of data, don't add a synchronous read of the same data on the main thread — a one-tick-stale async value is almost always preferable to main-thread blocking on a cross-process or otherwise-slow call. Concrete example: `AlarmManager.shared.alarmUpdates` makes a synchronous `AlarmManager.shared.alarms` read on `willEnterForeground` unnecessary.
